...
const API_KEY = 'c727cc269ea147d4926154115241408';
  ...
  hourlyWeatherDiv.innerHTML = next24HoursData
    .map((item) => {
      ...
      return `
        ...
        <p class="time">${time}</p>
        <img src="icons/${weatherIcon}.svg" alt="Clouds" class="weather-icon" />
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setupWeatherRequest(`${lat},${lon}`);
      },
      (error) => {
        console.error('Ошибка получения геолокации: ', error);
        setupWeatherRequest('Kyiv');
      },
    );
  }
};

// Вызов функции для получения местоположения и погоды
